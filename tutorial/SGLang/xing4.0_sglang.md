# 使用 SGLang 部署 Xing4.0-29B-A4B 模型

　　本文介绍如何通过 SGLang 启动 Xing4.0-29B-A4B 模型的 OpenAI 兼容 API 服务。主要覆盖以下内容：
...
　　Xing4.0 的 SGLang 适配代码（[sgl-project/sglang#39793](https://github.com/sgl-project/sglang/pull/39793)）目前尚未合入 SGLang 主分支。在合入前，本文使用已集成该 PR 的预编译镜像 `quay.io/xingchen-agi/xingchen-inference-sglang:v0.5.20.rc1-xing4_0`，无需自行从源码编译即可快速部署。
...
　　本文启动命令以 `--tp-size 2` 为例，需要至少 2 张 GPU；可根据实际卡数调整该参数。

> [!NOTE]
> 本文中的 NVIDIA H100（单卡 80GB 显存，2 卡共 160GB）仅为**演示与验证所用的硬件环境**，并非强制要求。用户可根据自身硬件选择其他支持 CUDA 的 NVIDIA GPU，只需根据实际显存与卡数调整 `--tp-size`、`--context-length`、`--mem-fraction-static` 等参数即可；显存有限时也可参考下文使用 FP8 版本模型。
...
　　如需进一步降低显存占用，也可使用 FP8 版本（[Hugging Face](https://huggingface.co/XingChen-AGI/Xing4.0-29B-A4B-FP8) | [ModelScope](https://modelscope.cn/models/XingChen-AGI/Xing4.0-29B-A4B-FP8) | [Modelers](https://modelers.cn/models/XingChen-AGI/Xing4.0-29B-A4B-FP8)），下载与启动方式与下方一致，仅需将模型路径指向 FP8 目录即可。
...
- 降低 `--mem-fraction-static`（如 0.85）或 `--context-length`（如 131072）
- 使用 FP8 版本模型以减少显存占用
- 减少 `--max-running-requests` 以降低并发显存峰值

### 张量并行卡数不匹配

- `--tp-size` 必须能整除可见 GPU 数；如使用 4 卡可设为 `4` 或 `2`
