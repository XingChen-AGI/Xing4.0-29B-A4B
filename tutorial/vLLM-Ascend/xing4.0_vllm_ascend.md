...
本文档说明如何在昇腾(Ascend)NPU 环境下，基于 vLLM-Ascend 推理框架部署并调用 **Xing4.0-29B-A4B** 大语言模型。
...
该镜像已内置针对 Xing4.0-29B-A4B 优化的 vLLM-Ascend 环境，并开启多项加速能力(详见第 7 节启动参数详解)。
...
- **存储**:
  ...
  - 草稿模型权重目录(仅 dspark 投机采样需要)，mtp 模式无需草稿模型，详见第 6 节。
...
| --- | --- | --- | --- | --- |
| 方式一 | dspark(可选) | 需要 | `8000` | `dspark.log` |
| 方式二 | mtp | 不需要(使用主模型 MTP 预测头) | `8009` | `mtp.log` |

### 6.1 方式一:dspark 投机采样(可选)

> **dspark 为可选项**:它依赖额外的草稿模型，不启用时服务同样可正常推理，仅吞吐略低。<br>若需启用，请确保草稿模型权重就绪，在--speculative-config的model字段替换为你的dspark草稿模型权重地址(见第 5 节)。
